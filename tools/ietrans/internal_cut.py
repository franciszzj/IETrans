...
for i, data in enumerate(l):
    ...
    sub_idxs, obj_idxs, rels = relation_tuple[:,
                                              0], relation_tuple[:, 1], relation_tuple[:, 2]
...
print(len(all_triplet_rels), len(all_triplet_subs),
      len(all_triplet_objs), all_triplet_logits.size())
assert len(all_triplet_rels) == len(all_triplet_subs) == len(
    all_triplet_objs) == len(all_triplet_logits)
...
def find_wrong_most_triplet(r):
    ...
    pair_dic = {pair: logit[int(pred2idx[r])]
                for pair, logit in pair_dic.items()}
...
for triplet in tqdm(all_triplets):
    ...
    parents = [p for p in parents if importance_dic.get(
        (p, triplet[1], triplet[2]), 0) < importance_dic[triplet]]

    # get data from parents
    # collect all parent data
    idxs, logits = collect_all_parent_data(
        parents, (triplet[1], triplet[2]), pred2idx[r])
